...
def get_ticker(query):
    ...
    try:
        ...
        data = yf.utils.get_json(url, params=params, user_agent="Mozilla/5.0")
...
with st.sidebar:
    ...
    polygon_key = st.text_input("XvjaFCA_pWPpLPWzgHl01ykGEowOdh1O (optional - for better data)", type="password")
    st.button("Run Analysis", type="primary")

if st.button("Run Analysis") or "data_loaded" in st.session_state:
    try:
        ...
        # Polygon integration (optional)
        hist_df = pd.DataFrame()
        if polygon_key:
            try:
                client = RESTClient(polygon_key)
                financials = list(client.vx.list_stock_financials(ticker=ticker.replace(".SR", ":SA") if ".SR" in ticker else ticker, timeframe="annual", limit=12))
                data = []
                for f in financials:
                ...
                hist_df = pd.DataFrame(data).dropna(how="all").sort_values("Year")
                st.info("✅ Polygon data loaded (10+ years historical)")
            except:
                st.warning("Polygon key valid but data not available for this ticker - using yfinance fallback")
        ...
        if hist_df.empty:
            ...
            if not income.empty:
                ...
                }).dropna(how="all").sort_values("Year")
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            pd.DataFrame([info]).to_excel(writer, sheet_name="Summary", index=False)
        ...
        st.error(f"Error: {str(e)} - Try another ticker or check API key")

st.caption("FinForecast AI • Powered by yfinance + Polygon • Clean English code")